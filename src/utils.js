...
const parseURL = function (url) {
    ...
    getType(url) == "String" && url.replace(/^.*?\?/, '').split('&').forEach(v => {
        let t = v.split('=');
        tmp[t[0]] = t[1] ? t[1] : '';
    });
    return tmp;
...
const getArrayFunction = function(items, args = []){
    if(!items) {
        return []
    }
    let set = []
    items.forEach(v => {
        if(getType(v) == 'Function') {
            ...
            if(Array.isArray(tmp)) {
                tmp.forEach(t => set.includes(t) ? '' : set.push(t))
            }else {
                set.includes(tmp) ? '' : set.push(tmp)
            }
        }else {
            set.includes(v) ? '' : set.push(v)
        }
    })
    return set
}
